...
describe('WorkspaceBrowser', () => {
  ...
  it('session delete dialog opens with session title, confirms, and calls deleteSession', async () => {
    const deleteSession = vi.fn(async () => {})
    const sessions = sessionState([summary('s-target', 2, { displayTitle: 'Target Session' })])
    mount({
      useSessions: hook(sessions),
      useWorkspaces: hook(workspaceState([workspace('alpha', ['s-target'])])),
      deleteSession,
    })
    fireEvent.click(screen.getByText('alpha'))
    fireEvent.click(screen.getByRole('button', { name: '会话“Target Session”的操作' }))
    fireEvent.click(screen.getByRole('menuitem', { name: '删除会话' }))

    const dialog = screen.getByRole('dialog', { name: '删除会话' })
    expect(dialog).toBeTruthy()
    expect(screen.getByText(/将永久删除会话“Target Session”及其所有历史记录/)).toBeTruthy()

    await act(async () => {
      fireEvent.click(screen.getByRole('button', { name: '确认删除' }))
    })
    expect(deleteSession).toHaveBeenCalledWith(sid('s-target'))
    expect(screen.queryByRole('dialog', { name: '删除会话' })).toBeNull()
  })

  it('session delete dialog can be cancelled without calling deleteSession', () => {
    const deleteSession = vi.fn(async () => {})
    const sessions = sessionState([summary('s-target', 2, { displayTitle: 'Target Session' })])
    mount({
      useSessions: hook(sessions),
  ...
  it('session delete error renders in the modal alert', async () => {
