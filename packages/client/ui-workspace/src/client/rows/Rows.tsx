...
export function SessionNodeItem({ node, currentId, now, onOpen, onRename, onFork, onArchive, onDelete, drag, flat = false, t }: {
  ...
  /** Delete this session permanently (row menu action; opens confirmation dialog). */
  ...
  const sessionMenuItems = [
    ...
    { id: 'delete', label: t('delete.session'), icon: <IconTrashOutline16 /> },
  ]
        ...
        <span className={css.rowActions}>
          <Menu
            ...
            onSelect={(id) => {
              ...
              if (id === 'delete' && onDelete) onDelete(node.id, row.title)
            }}
            portal
            closeOnPointerLeave
            anchor={(
