...
export function apply(ctx: ClientContext): void {
  ...
  const browserInjected = (): WorkspaceBrowserInjected => ({
    ...
    deleteSession: async (sessionId) => {
      // 1. If currently opened session is being deleted, clear the active selection
      if (ctx.sessions.list.getSnapshot().current === sessionId) {
        ctx.sessions.clear()
      }
      // 2. Hide and remove from workspace group lists
      await ctx.workspaces.archiveSession(sessionId)
      // 3. Attempt physical backend deletion if available
      try {
        const binding = ctx.sessions.binding(sessionId)
        if (binding?.session && typeof (binding.session as any).delete === 'function') {
          await (binding.session as any).delete()
        }
      } catch (err) {
        console.warn('Physical session delete:', err)
      }
    },
