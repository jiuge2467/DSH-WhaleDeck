...
const zstdCompressFn = (zlib as { zstdCompress?: any }).zstdCompress
const zstdDecompressFn = (zlib as { zstdDecompress?: any }).zstdDecompress
const zstdCompressAsync = typeof zstdCompressFn === 'function' ? promisify(zstdCompressFn) : null
const zstdDecompressAsync = typeof zstdDecompressFn === 'function' ? promisify(zstdDecompressFn) : null

const zlibConstants = (zlib as { constants?: Record<string, number> }).constants || {}
const CHECKSUM_OPTIONS: any = {
  params: { [zlibConstants.ZSTD_c_checksumFlag || 0]: 1 },
}
const INCOMPLETE_FRAME_OPTIONS: any = {
  finishFlush: zlibConstants.ZSTD_e_flush || 0,
}
...
export async function compressZstdFrame(input: Buffer | string): Promise<Buffer> {
  if (!zstdCompressAsync) {
    throw new Error('Zstandard compression is not supported in this Node.js runtime')
  }
