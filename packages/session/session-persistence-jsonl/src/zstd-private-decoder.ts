...
type NodeZstdPrivateStream = any & NodeZstdPrivateState
...
export class NodePrivateZstdFrameDecoder implements ZstdFrameDecoder {
  ...
  static create(): NodePrivateZstdFrameDecoder | undefined {
    const createFn = (zlib as { createZstdDecompress?: (opts?: { chunkSize?: number }) => unknown }).createZstdDecompress
    if (typeof createFn !== 'function') return undefined
    try {
      const stream = createFn({ chunkSize: DECODE_CHUNK_SIZE }) as any
