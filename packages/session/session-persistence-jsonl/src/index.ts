    ...
    for (const project of await this.listProjectDirs(signal)) {
      ...
      for (const dir of await this.listSessionDirs(project, signal)) {
        ...
        try {
          ...
          // Skip sessions whose format this runtime cannot read (e.g. Zstd logs on
          // a Node.js build that predates zlib Zstd support). A format refusal is
          // not corruption — the log remains intact for a capable runtime.
    ...
    try {
      ...
      for (;;) {
        ...
        try {
          ...
          /* v8 ignore next -- decoder failure plus concurrent abort is timing-dependent */
          if (signal?.aborted) signal.throwIfAborted()
          // When the runtime lacks Zstd support, surface a typed refusal so
          // listArtifacts can skip this session instead of crashing the process.
          if (error instanceof Error && error.message.includes('not supported in this Node.js runtime')) {
            throw new SessionFormatUnsupportedError(
              `Zstandard session log requires a Node.js runtime with zlib Zstd support (Node >= 22.15): ${path}`,
            )
          }
