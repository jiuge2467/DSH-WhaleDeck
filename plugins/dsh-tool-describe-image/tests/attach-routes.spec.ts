...
import { attachmentMarkdown, attachmentNote, attachmentRefById, handleAttach, handleDiscoverModels, handleTestConnection, registerAttachRoute, registerAttachmentRef, validateAttachPayload, type AttachError } from '../src/attach-routes.ts'
...
describe('registerAttachRoute', () => {
  ...
  it('handles /describe-image/discover-models endpoint successfully', async () => {
    const { jsonReply, startMockServer } = await import('./mock-server.ts')
    ...
    await registrations[0].handler(
      makeReq('POST', JSON.stringify({ baseURL: mock.url, apiKey: 'sk-test' }), '/describe-image/discover-models'),
      res,
    )
    expect(status()).toBe(200)
    const envelope = JSON.parse(body()) as { ok: boolean; models: Array<{ id: string; isVisionRecommended: boolean }> }
    expect(envelope.ok).toBe(true)
    expect(envelope.models).toHaveLength(3)
    expect(envelope.models[0].isVisionRecommended).toBe(true)
    expect(envelope.models.find(m => m.id === 'deepseek-chat')?.isVisionRecommended).toBe(false)
    await mock.close()
  })

  it('handles /describe-image/test-connection endpoint successfully', async () => {
    const { chatReply, jsonReply, startMockServer } = await import('./mock-server.ts')
    const mock = await startMockServer((req, res) => {
      if (req.path === '/chat/completions') {
        jsonReply(res, 200, chatReply('pong'))
      }
    })
    ...
    const { res, status, body } = makeRes()
    await registrations[0].handler(
      makeReq('POST', JSON.stringify({ baseURL: mock.url, model: 'test-model', apiKey: 'sk-test' }), '/describe-image/test-connection'),
      res,
    )
    expect(status()).toBe(200)
    const envelope = JSON.parse(body()) as { ok: boolean; latencyMs: number; text: string }
    expect(envelope.ok).toBe(true)
    expect(envelope.latencyMs).toBeGreaterThanOrEqual(0)
    expect(envelope.text).toBe('pong')
    await mock.close()
  })

  it('rejects test-connection when model is missing', async () => {
    const registrations = capture(undefined, true)
    const { res, status, body } = makeRes()
    await registrations[0].handler(
      makeReq('POST', JSON.stringify({ baseURL: 'https://api.example.com/v1', model: '' }), '/describe-image/test-connection'),
      res,
    )
    expect(status()).toBe(400)
    const envelope = JSON.parse(body()) as { ok: boolean; error: string }
    expect(envelope.ok).toBe(false)
    expect(envelope.error).toContain('模型 ID 不能为空')
  })
})
