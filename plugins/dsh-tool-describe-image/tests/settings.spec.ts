...
describe('describe-image settings section', () => {
  ...
  it('allows updating only baseURL without throwing model required error', async () => {
    const { ctx } = await boot()
    await expect(ctx.settings.update(tool.DESCRIBE_IMAGE_SETTINGS_NAMESPACE, { baseURL: 'https://new-api.example.com/v1' }))
