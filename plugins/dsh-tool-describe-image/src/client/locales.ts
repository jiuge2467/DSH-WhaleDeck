...
export const zh = {
  ...
  'field.model.fetch': '获取模型列表',
  'field.model.fetching': '获取中…',
  'field.model.fetchSuccess': '已获取 {count} 个模型',
  'field.model.fetchEmpty': '未找到可用模型',
  'field.model.fetchError': '获取失败: {error}',
  'field.model.recommended': '推荐视觉模型',
  'field.model.other': '其他模型',
  'field.model.custom': '自定义输入',
  'test.button': '测试连通性',
  'test.testing': '正在测试连通性…',
  'test.success': '连通正常 (HTTP 200, 耗时: {latency}ms)',
  'test.error': '测试失败: {error}',
  'test.needBaseURL': '请先填写接口地址',
  'test.needModel': '请先填写或选择模型',
...
export const en = {
  ...
  'field.model.fetch': 'Fetch Models',
  'field.model.fetching': 'Fetching…',
  'field.model.fetchSuccess': 'Found {count} models',
  'field.model.fetchEmpty': 'No models found',
  'field.model.fetchError': 'Fetch failed: {error}',
  'field.model.recommended': 'Recommended Vision Models',
  'field.model.other': 'Other Models',
  'field.model.custom': 'Custom Model',
  'test.button': 'Test Connection',
  'test.testing': 'Testing connection…',
  'test.success': 'Connection healthy (HTTP 200, {latency}ms)',
  'test.error': 'Test failed: {error}',
  'test.needBaseURL': 'Please enter a Base URL first',
  'test.needModel': 'Please enter or select a model first',
} satisfies Record<string, string>
...
function format(template: string, params: Record<string, unknown>): string {
  return template.replace(/\{([a-zA-Z0-9]+)\}/g, (match, name: string) => {
    ...
    if (typeof val === 'object' && val !== null) {
      try {
        return (val as { message?: string }).message || JSON.stringify(val)
      } catch {
        return String(val)
      }
    }
...
export function t(key: DescribeImageClientKey, params?: Record<string, unknown>): string {
