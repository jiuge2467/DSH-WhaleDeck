...
import { API_STYLES, Config, DESCRIBE_IMAGE_SETTINGS_NAMESPACE, resolveApiKey, resolveConfig, type ResolvedConfig } from './config-resolve.ts'
...
export function apply(ctx: Context, config: Config = {}): void {
  ...
  installSettingsSection(ctx, DESCRIBE_IMAGE_SETTINGS_NAMESPACE, Config, config, {
    ...
    onChange: () => {},
    validate: (value) => {
      if (value.baseURL !== undefined && value.baseURL.trim() !== '') {
        if (!/^https?:\/\//i.test(value.baseURL.trim())) {
          throw new Error('describe-image: baseURL must be an absolute http(s) URL')
        }
      }
      if (value.apiKey !== undefined && value.apiKey.length === 0) {
        throw new Error('describe-image: apiKey must be non-empty when set')
      }
      if (value.maxBytes !== undefined && (!Number.isSafeInteger(value.maxBytes) || value.maxBytes <= 0)) {
        throw new Error('describe-image: maxBytes must be a positive safe integer')
      }
      if (value.maxOutputTokens !== undefined && (!Number.isSafeInteger(value.maxOutputTokens) || value.maxOutputTokens <= 0)) {
        throw new Error('describe-image: maxOutputTokens must be a positive safe integer')
      }
      if (value.timeoutMs !== undefined && (!Number.isSafeInteger(value.timeoutMs) || value.timeoutMs <= 0)) {
        throw new Error('describe-image: timeoutMs must be a positive safe integer')
      }
      if (value.apiStyle !== undefined && !API_STYLES.includes(value.apiStyle)) {
