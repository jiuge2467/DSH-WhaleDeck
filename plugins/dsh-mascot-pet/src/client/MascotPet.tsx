...
export function MascotPet({
  ...
  useEffect(() => {
    const handleSettingsChange = (e: Event) => {
      ...
      if (typeof detail.activeSkin === 'string' && detail.enabled !== false) {
  ...
  }, [wanderEngine, enabled, wanderEnabled, panelOpen, running, stateMachine])
  ...
  }, [enabled, running, liveTokenData.toolName, usage, stateMachine, wanderEngine])

  // 6. 闲置随机微动作调度器 (打瞌睡、伸懒腰、吹泡泡)
  useEffect(() => {
    if (!enabled || running || panelOpen) return
  ...
  }, [enabled, running, panelOpen, stateMachine, affectionStore])
  ...
  }, [enabled, running, panelOpen, dialogQuote, affectionStore])
