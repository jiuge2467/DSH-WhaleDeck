...
export const MascotTokenBridge = memo(function MascotTokenBridge(props: MascotTokenBridgeProps) {
  ...
  // 2. 读取会话状态、Session ID、活跃工具与节点列表、模型名称
  ...
  const sessionModel = useSession
    ? useSession((s: any) => s?.model ?? s?.modelId ?? s?.spec?.model ?? s?.config?.model)
    : undefined
  ...
  useEffect(() => {
    ...
    if (Array.isArray(settledNodes)) {
      for (const node of settledNodes) {
        if (node && typeof node === 'object') {
          ...
          if (node.reading && typeof node.reading.reasoningTokens === 'number') {
            fallbackReasoning += node.reading.reasoningTokens
          }
        }
      }
    }
    ...
    // 提取或构造真实 usage
    const finalUsage: TokenUsageLike | undefined = tokenUsage ? {
      uncachedInputTokens: tokenUsage.uncachedInputTokens ?? 0,
      cacheReadTokens: tokenUsage.cacheReadTokens ?? 0,
      cacheWriteTokens: tokenUsage.cacheWriteTokens ?? 0,
      outputTokens: tokenUsage.outputTokens ?? 0,
      reasoningTokens: tokenUsage.reasoningTokens ?? (tokenUsage as any)?.completion_tokens_details?.reasoning_tokens ?? fallbackReasoning,
    } : (fallbackOutput > 0 || fallbackInput > 0 ? {
      uncachedInputTokens: fallbackInput,
      cacheReadTokens: 0,
      outputTokens: fallbackOutput,
      reasoningTokens: fallbackReasoning,
    } : undefined)

    if (sessionId && sessionId !== 'default') {
      recordSessionCostToLedger(sessionId, finalUsage, undefined, sessionModel)
    }
