...
describe('Client Components', () => {
  ...
  it('should completely unmount MascotPet when disabled in localStorage', async () => {
    localStorage.setItem('dsh_mascot_enabled', 'false')
    const { MascotPet } = await import('../src/client/MascotPet.js')
    const container = document.createElement('div')
    document.body.appendChild(container)
    const root = createRoot(container)

    root.render(React.createElement(MascotPet, {}))
    ...
    return new Promise<void>((resolve) => {
      setTimeout(() => {
        ...
        root.unmount()
        container.remove()
        localStorage.removeItem('dsh_mascot_enabled')
        resolve()
      }, 50)
    })
  })
